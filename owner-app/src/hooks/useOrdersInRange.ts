...
import { subscribeToOrdersInRange, type Order } from "@kumbakonam/shared";
...
export function useOrdersInRange(range: DateRange): UseOrdersInRangeResult {
  ...
  useEffect(() => {
    setLoading(true);
    const unsubscribe = subscribeToOrdersInRange(new Date(startTime), new Date(endTime), (result) => {
      setOrders(result);
      setLoading(false);
    });
    return unsubscribe;
